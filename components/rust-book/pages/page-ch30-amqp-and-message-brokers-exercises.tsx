...
const exercises: Exercise[] = [
  ...
  },
  {
    number: 2,
    kind: "code reading",
    title: "Trace the idempotent consumer's state transitions",
  ...
  },
  {
    ...
    starterPrompt: "Starting from the DirectExchange struct in the main chapter's Example 1 (no unrouted counter, &self on route), add a method that supports a single trailing wildcard segment, so binding orders.* reaches a queue for both orders.created and orders.cancelled. Keep the exact-match route method working unchanged.",
    prompts: [
  ...
  },
  {
    ...
    starterPrompt: "Start from this broken consumer, which acks before it commits and requeues every failure forever:\n\n    fn handle(&mut self, id: &str, payload: &str) {\n        self.ack(id);                 // BUG: acks before the durable write\n        if let Err(_) = self.commit(id, payload) {\n            self.requeue(id);         // BUG: unbounded retry, even for malformed payloads\n        }\n    }\n\nIdentify both defects against the chapter's guidance and refactor the handler.",
    prompts: [
  ...
  },
  {
    number: 7,
    kind: "implementation",
    title: "Bound the local worker budget, not just broker prefetch",
    objective: "Show that a broker prefetch cap without a bounded in-process worker queue just moves the overload into the process.",
    starterPrompt: "Model a consumer with two separate limits: a broker prefetch cap (how many unacked messages it will hold) and a bounded local worker queue (how much in-process work those messages can spawn). Feed it a burst larger than the local queue and demonstrate that the local cap, not prefetch, is what protects the process from unbounded growth.",
    prompts: [
      "Give the consumer a prefetch field and a separate bounded local queue with its own capacity.",
      "When the broker offers a message, honor prefetch first, then refuse to enqueue past the local capacity, counting the shed work instead of letting the buffer grow.",
      "Set prefetch generously (say 8) but the local capacity small (say 2), feed a burst of 5, and report how many were buffered, how many were rejected, and how many were eventually worked.",
      "Explain in one sentence why setting prefetch alone, without the local cap, would only move the overload from the broker into the process heap.",
    ],
    acceptanceCriteria: [
      "The consumer carries a broker prefetch cap and a bounded local worker queue as two independent limits.",
      "A message offered past the local capacity is shed and counted rather than appended to an unbounded buffer.",
      "With prefetch 8, local capacity 2, and a burst of 5, the output is buffered = 2, rejected = 3, done = 2.",
      "The answer states that prefetch without a local cap relocates the overload into the process rather than removing it.",
    ],
    hints: [
      "Prefetch and local capacity are two layers of backpressure; setting one without the other only moves where the work piles up.",
      "Check the local queue length before pushing; if it is at capacity, increment a rejected counter instead of growing the buffer.",
    ],
  },
]
