...
// Chapter 28 · exercise workbook page (ch28-cpp-integration-exercises).
...
// Intended component name: PageCh28CppIntegrationExercises
...
const exercises: Exercise[] = [
  {
    ...
    title: "Sort types into what may cross the seam",
    objective: "Distinguish C ABI-stable representations from Rust-specific types whose layout must stay behind the boundary.",
    starterPrompt: "The chapter's central filter is to put every type into one of two buckets: a stable, agreed-upon shape that may cross a C ABI seam, or a Rust-specific type whose layout is an implementation detail. Sort the following: i32, &str, *const i32 plus a usize length, Vec<u8>, *mut i64, Result<u32, MyError>, and an opaque handle pointer.",
    prompts: [
  ...
  },
  {
    number: 2,
    kind: "code reading",
    title: "Trace the three layers of the abs wrapper",
    objective: "Read the chapter's calling-C-from-Rust example and identify which single layer is unsafe and why.",
    starterPrompt: "Re-read Example 1, where ffi_demo_abs is defined with extern \"C\" and no_mangle, reached through an unsafe extern \"C\" declaration, called inside the one-line wrapper safe_abs, and finally used from main. The body uses wrapping_abs rather than abs.",
    prompts: [
  ...
  },
  {
    number: 3,
    kind: "implementation",
    title: "Export a length-prefixed buffer reader",
    objective: "Build a C ABI-safe exported function that takes a raw pointer plus length, validates null, and reports failure through an integer status.",
    starterPrompt: "Following the shape of sum_i32s from Example 2, write a function fn max_i32(ptr: *const i32, len: usize, out_max: *mut i32) -> i32 that writes the maximum element through the out parameter and returns 0 on success.",
    prompts: [
  ...
  },
  {
    number: 4,
    kind: "implementation",
    title: "Map ownership across the seam with an opaque handle",
    objective: "Express Rust-owned memory across a C ABI boundary using an opaque handle and an explicit destroy function.",
    starterPrompt: "Design a tiny C ABI surface for a Rust-owned counter: a create function returning an opaque handle, an increment function taking that handle, a read function, and a destroy function. The Rust struct behind the handle must never appear in the C signatures.",
    prompts: [
  ...
  },
  {
    ...
    title: "Choose a binding tool for a legacy C++ subsystem",
    objective: "Apply the chapter's decision tree to select between a hand-written C shim, bindgen, cxx, and autocxx for a real integration.",
    starterPrompt: "A product is replacing one hot native subsystem with Rust while keeping the existing host process, plugin loader, and native test harness. You must reach a large legacy C++ surface, you control the Rust side but not the historical C++ headers, and the requirement is a narrow ABI contract with explicit layout, pointer validity, ownership, status-code errors, and no cross-language unwinding.",
    prompts: [
...
<RustPracticeCard
  title={"Runnable lab · status-code seam over a caller buffer"}
