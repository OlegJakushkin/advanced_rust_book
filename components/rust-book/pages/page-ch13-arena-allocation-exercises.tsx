...
const exercises: Exercise[] = [
  ...
  },
  {
    ...
    acceptanceCriteria: [
      ...
      "The answer explains that ExprId is logical identity into one owning vector, not shared ownership, so edges are indices rather than owners, and states that switching Add to Box<Expr> would make each node own its children exclusively, preventing sub-expression sharing and giving a tree-only (not DAG) structure.",
    ],
    hints: [
  ...
  },
  {
    ...
    hints: [
      ...
      "rewind is only valid when the saved mark is less than or equal to the current used cursor; guard or document the case where mark exceeds the live cursor.",
    ],
  },
  {
    number: 4,
  ...
  },
  {
    ...
    starterPrompt: "You are given a small graph of Rc<RefCell<Node>> where edges are Rc<RefCell<Node>> owners and a back-edge from D to A forms a cycle that never drops to zero. Refactor it so one arena owns all nodes and every edge is a NodeId index.",
    prompts: [
...
const reviewQuestions = [
  ...
  "The arena's only cheap teardown operation is whole-region reset, not per-object free. When does that make it the wrong fit?",
  "Given the chapter's three-question framework, which crate family fits a build-once AST and which fits a deleting registry that needs stale-handle protection?",
]
